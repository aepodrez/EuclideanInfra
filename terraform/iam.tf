    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          # Predictor Lambdas (signal-master + 66 predictors) invoked by the decision pipeline.
          "arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:${var.project_name}-pred-*",
        ]
      },
      {
        Effect = "Allow"
